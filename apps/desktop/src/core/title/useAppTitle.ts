...
export function useAppTitle() {
  ...
  async function setTitle(newTitle: string, updateNativeTitle = false) {
    ...
    if (updateNativeTitle) {
      try {
        ...
        logError('Failed to set window title', { error: err as Error } );
      }
    }
  }
  ...
  async function setEditorTitle(
    ...
    if (
      fileExt &&
      baseFileName.toLowerCase().endsWith(`.${fileExt.toLowerCase()}`)
    ) {
    ...
    if (workspaceName) {
      ...
      finalTitle = `${unsavedIndicator}${displayFileName} - ${DEFAULT_TITLE}`;
    }
