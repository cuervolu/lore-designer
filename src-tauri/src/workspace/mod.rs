...
impl WorkspaceManager {
    ...
    pub fn create_workspace(
        ...
        if workspace_path.exists() {
            warn!("Workspace directory already exists: {}", workspace_path.display());
            return Err(AppError::DirectoryExists(workspace_path.display().to_string()));
        }
        ...
        info!("Workspace '{}' created successfully at {}", name, workspace_path.display());
        Ok(workspace_path)
    }
    ...
    fn create_directory_structure(workspace_path: &Path) -> Result<(), AppError> {
        ...
        fs::create_dir_all(workspace_path)
            .context(format!("Failed to create workspace directory: {}", workspace_path.display()))?;
        debug!("Created workspace directory: {}", workspace_path.display());
        
        let dot_lore_path = workspace_path.join(INTERNAL_DIR);
        fs::create_dir(&dot_lore_path)
            .context(format!("Failed to create .lore directory: {}", dot_lore_path.display()))?;
        debug!("Created internal '.lore' directory");
        
        for subdir in INTERNAL_SUBDIRS.iter() {
            let subdir_path = dot_lore_path.join(subdir);
            fs::create_dir(&subdir_path)
                .context(format!("Failed to create subdir: {}", subdir_path.display()))?;
        ...
        for folder in DEFAULT_FOLDERS.iter() {
            ...
            fs::create_dir(&folder_path)
                .context(format!("Failed to create content folder: {}", folder_path.display()))?;
